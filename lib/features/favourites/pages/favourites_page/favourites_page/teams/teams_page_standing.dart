...
import 'package:blogapp/state/bloc/mirchaweche/my_fav/my_fav_player/myfavourite_players_event.dart';
...
import 'package:blogapp/state/bloc/mirchaweche/teams/previous&next_matchs/match_page_state.dart';
import 'package:blogapp/state/bloc/mirchaweche/teams/previous&next_matchs/matches_bloc.dart';
import 'package:blogapp/state/bloc/mirchaweche/teams/previous&next_matchs/matches_page_event.dart';
import 'package:blogapp/state/bloc/mirchaweche/teams/team_profile_standing/team_profile_standing_bloc.dart';
import 'package:blogapp/state/bloc/mirchaweche/teams/team_profile_standing/team_profile_standing_state.dart';
import 'package:blogapp/state/bloc/mirchaweche/teams/last_five_matches/last_five_matches_bloc.dart';
import 'package:blogapp/state/bloc/mirchaweche/teams/last_five_matches/last_five_matches_event.dart';
import 'package:blogapp/state/bloc/mirchaweche/teams/last_five_matches/last_five_matches_state.dart';
...
// FORM ITEM WIDGET
  ...
  String _normalizeTeamId(dynamic teamId) {
    final raw = teamId?.toString().trim() ?? '';
    ...
    if (parsedNum != null) {
      return parsedNum.toInt().toString();
    }

    return RegExp(r'\d+').firstMatch(raw)?.group(0) ?? '';
  }

  String _extractTeamIdFromLogoUrl(String? logoUrl) {
    final url = logoUrl?.trim() ?? '';
    if (url.isEmpty) return '';

    final directTeamMatch =
        RegExp(r'/teams/(\d+)\.png', caseSensitive: false).firstMatch(url);
    if (directTeamMatch != null) {
      return directTeamMatch.group(1) ?? '';
    }

    return RegExp(r'\d+').firstMatch(url)?.group(0) ?? '';
  }

  String _teamLogoById(dynamic teamId, {String? sourceLogoUrl}) {
    final normalizedId = _normalizeTeamId(teamId);
    final fallbackId = normalizedId.isNotEmpty
        ? normalizedId
        : _extractTeamIdFromLogoUrl(sourceLogoUrl);
    if (fallbackId.isEmpty) return '';
    return 'https://media.api-sports.io/football/teams/$fallbackId.png';
  }

  String _resolveTeamLogoUrl(String? logoUrl, dynamic teamId) {
    final cleanedLogo = logoUrl?.trim() ?? '';
    if (cleanedLogo.isNotEmpty &&
        cleanedLogo.toLowerCase() != 'null' &&
        cleanedLogo.toLowerCase() != 'undefined') {
      return cleanedLogo;
    }
    return _teamLogoById(teamId, sourceLogoUrl: cleanedLogo);
  }

  Widget _buildTeamColumn(String? logoUrl, String teamName, dynamic teamId) {
    final fallbackUrl = _teamLogoById(teamId, sourceLogoUrl: logoUrl);
    final imageUrl = _resolveTeamLogoUrl(logoUrl, teamId);
